...
double sqdistance(const Point &p1, const Point &p2) {
	double dx = p1.x - p2.x;
	double dy = p1.y - p2.y;
	return dx*dx + dy*dy;
}
...
int main (int argc, char* argv[])
{
	...
	Result res = getRes(pm);

	bool final = res.valid;
	for(int i = 0; i < m; ++i) {
		if(!(sqdistance(ps[i],res.c) > res.sr)) {
			final = false;
			break;
		}
	}

	Result res2 = getRes(ps);
	bool final2 = res.valid;
	for(int i = 0; i < n; ++i) {
		if(!(sqdistance(pm[i],res2.c) > res2.sr)) {
			final2 = false;
			break;
		}
	}
